...
.controller('schedulerUpdateSiteController',function(schedulerHttpService,utilFactory,$stateParams,$state,$scope){
...
//   </div>
     ...
     $(document).ready(function(){
        // $("#stationType option[value='上行']").attr("selected","selected");
        setTimeout(function(){
           $("#stationType").val($scope.stationType)
        },200)
         //console.log('xxxx'+$("#stationType").find("option[text='上行']"))
     })

      var _formContentTmpl = '<div class="form-item"><input type="text" name="name"  id="name" value="'+$scope.stationName+'"></div>'+
                              '<div class="form-item"><input type="text" name="address" id="address" value="'+$scope.address+'"></div>'+
                              '<div class="form-item"><select name="stationType" id="stationType"><option value="AM" >上行</option><option value="PM">下行</option></select></div>'+
                              '<div class="form-item"><button>取消</button><button class="confirmBtn">确认</button></div>';
       var title = '更新站点';
